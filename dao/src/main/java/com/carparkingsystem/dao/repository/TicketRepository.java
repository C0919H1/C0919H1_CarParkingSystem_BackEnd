...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    @Query(value = "SELECT YEAR(t.ticketregistrationdate) AS NAM, SUM(DATEDIFF(t.enddate, t.startdate)* tp.cost) AS DOANHTHU FROM ticket t, tickettype tp WHERE t.idtickettype = tp.idtickettype GROUP BY NAM", nativeQuery = true)
    List getRevenue();
}
